...
const LanguagePack = ({ showToggler, forwardedRef, visible }: ILanguagePack) => {
   ...
   const renderLanguageTiles = () => languages.map(l => (
      <div
         className={
      ...
      >
         <ReactCountryFlag svg countryCode={l.country} aria-label={l.text} /><span className='language-pack__lang'>{l.text}</span>
      </div>
   ))

   return (
      <div className="language-pack" style={{ borderBottomRightRadius: visible ? 0 : '2.7rem' }} title="ENG / PL" onClick={showToggler} ref={forwardedRef}>
